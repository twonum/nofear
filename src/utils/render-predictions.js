...
export const renderPredictions = (predictions, ctx) => {
    ...
    predictions.forEach((prediction) => {
        ...
        if (isObjectInList) {
            ...
            // Send email alert if the object is in the list and email throttling allows
            if (canSendEmail()) {
...
/**
 * Send an email alert with a screenshot attachment.
 * @param {HTMLCanvasElement} canvas - The canvas element.
 * @param {Object} prediction - The detected object prediction.
...
const sendEmailAlert = async (canvas, prediction, userEmail) => {
    try {
        ...
        // Send the email via API (make sure the endpoint is correct)
        const response = await fetch("/api/send-email", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                to: userEmail, // Retrieved from auth service
